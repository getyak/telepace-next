...
function buildRespondents(studyId: string): Respondent[] {
  ...
  return profiles.map((p, i) => {
    ...
    const dateStr = `2025-03-${String(10 + idx).padStart(2, "0")}`;
    const turns: Turn[] = [
      { id: turnId(idx, 1), interview_id: intId, order: 1, role: "interviewer", text: "Thanks for joining. Can you tell me about your current research workflow?", started_at: `${dateStr}T10:00:00Z` },
      { id: turnId(idx, 2), interview_id: intId, order: 2, role: "respondent", text: `As a ${p.segments.role}, I spend most of my time coordinating with stakeholders and synthesizing findings manually.`, started_at: `${dateStr}T10:02:00Z` },
      { id: turnId(idx, 3), interview_id: intId, order: 3, role: "interviewer", text: "What's the biggest pain point in that process?", started_at: `${dateStr}T10:03:00Z` },
      { id: turnId(idx, 4), interview_id: intId, order: 4, role: "respondent", text: "Honestly, the time it takes to go from raw interviews to a shareable report. It's weeks, not days.", started_at: `${dateStr}T10:05:00Z` },
    ];
    ...
    const interview: Interview = {
      ...
      channel: p.channel,
      status: "completed",
      goal_coverage: 0.85 + idx * 0.01,
      turns,
      started_at: turns[0].started_at,
      completed_at: turns[turns.length - 1].started_at,
    };

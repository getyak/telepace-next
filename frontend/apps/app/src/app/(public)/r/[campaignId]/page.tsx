      ...
      if (msg.type === VoiceEventType.Error) {
        ...
        setMessages((prev) => [
          ...prev.filter((m) => !m.pending),
          {
            ...
            role: "system",
            text: "Something hiccuped on our side — please send that again.",
          },
        ]);
      }
    };
      ...
      try {
        ...
        setMessages((prev) => [
          ...prev,
          {
            id: crypto.randomUUID(),
            role: "system",
            text: "We couldn't access your microphone — you can keep typing instead.",
          },
        ]);
      }
    };
    ...
    <div className="min-h-screen flex flex-col">
      ...
      {dropped && (
        <div className="mx-auto w-full max-w-2xl px-6">
          <div className="mb-2 flex items-center justify-between rounded-card border border-hairline bg-paper-sunken px-4 py-2.5">
            <p className="text-sm text-body">Connection lost — your previous answers are saved.</p>
            <button
      ...
      <div className="w-full max-w-2xl mx-auto">
        <ChatComposer
          ...
          placeholder={awaiting ? "…" : "Type your reply…"}
        />
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-6">
      <div className="max-w-lg text-center">
        ...
        <p className="text-body text-lg leading-relaxed">
          {answered > 0
            ? `Your ${answered} answers are on their way to the research team — they read every word.`
            : "You've helped shape what gets built. The team will read every word."}
        </p>
        <p className="text-xs text-muted mt-6">You can close this tab now.</p>
      </div>
    </div>
  );
}
    ...
    <div className="mx-auto w-full max-w-2xl px-6 py-6">
      <div className="mb-2 flex items-baseline justify-between">
        <p className="overline">
          Question {current} of {progress.total}
        </p>

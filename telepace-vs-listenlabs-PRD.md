...
### P0 — 阻断 / 基线（1–2 周）
- [x] **修复创建流程崩溃**：`src/lib/errors.ts` 的 `copyFor` 对未知错误码兜底（不再读 undefined.title）；`handleSend` 失败时重置 loading + 展示错误 + 重试。（`TASKS.md` T-001/T-003/T-004）
- [x] **补全 zh 语言包 `errors` 命名空间**，消除 `MISSING_MESSAGE` 运行时报错。（`TASKS.md` T-002）
- [x] **创建失败的用户可见反馈**（错误文案 + loading 超时 + 重试）。（`TASKS.md` T-003）

### P1 — 对标基线补齐（3–6 周）
- [x] **分析报告引擎**：洞察 → 叙事报告（章节 + 目录锚点 + 可下载）。（`TASKS.md` T-102）
- [x] **图表 + 交叉分析**（与米色主题一致的图表皮肤）+ 统计规范（N/多选/小样本告警）。（`TASKS.md` T-103/T-104）
- [x] **证据底座（Evidence Graph）**：claim↔evidence 可追溯，报告/洞察引用可点击回溯到人。（`TASKS.md` T-101/T-105）
- [x] **响应级数据表**：逐人质量分/分群标签/Bullet Summary/筛除原因 + 搜索/筛选/导出。（`TASKS.md` T-106）
- [x] **单研究 Research Agent**：对话式问答，带 reasoning trace + 引用。（`TASKS.md` T-107）
- [x] **创建向导化**：分步 + 领域感知多轮追问 + 变更追踪 + 版本历史。（`TASKS.md` T-108/T-109/T-110/T-111）

### P2 — 差异化放大（6–12 周）
- [x] **收件箱运营 agent 体系化**（escalation/insight/progress/system + 一键动作 + 可审计）。（`TASKS.md` T-201）
- [x] **语音/电话原生分析**（情绪/语速/音频片段回放 + 转写对齐）+ 音频 Clips 自动剪辑。（`TASKS.md` T-202/T-203）
- [x] **MCP 研究工具化**（发起研究/查询洞察/导出片段暴露为 MCP 工具）。（`TASKS.md` T-204）
- [x] **跨研究 Copilot**（对标 Research Library）+ 一键落 Notion/Linear 闭环。（`TASKS.md` T-205）
- [ ] **外部受众供给 / 招募**，解决冷启动。（未开始，故意后置——见 `docs/issue-pricing-and-positioning-epic.md` T-633/T-643）
- [x] **i18n 一致性**（彻底双语或统一中文）。（`TASKS.md` T-206）

...
- [x] **T-512 · [增强] 注册/登录支持 SSO（Google / GitHub）**
...
2026-07-18 T-512 done — [增强]:Google SSO 登录/注册端到端接通(前端 UI 之前已有 GoogleButton/flag，本次补齐后端 OAuth 流 + BFF 桥接 + DB 迁移)。后端:①users 表迁移 password_hash 改可空 + 新增 auth_provider/provider_sub 列 + 部分唯一索引(启动即 ALTER，幂等);users_repo 加 create_oauth(无密码)/get_by_provider;②新增 auth/oauth_google.py——/start 用 jwt 签 5min state(防 CSRF)302 跳 Google，/exchange(BFF 服务端调)校验 state→httpx 换 token→userinfo→find-or-create(新用户 mint 独立 org 复用 T-501、老 provider_sub 复用账号、同邮箱密码账号绑定 provider 不建二租户)→签发 JWT;③login 加固:password_hash 为 None 的 SSO 账号拒绝密码登录(401，不泄露存在性);④config 加 google_client_id/secret/redirect_uri。前端:BFF /api/auth/oauth/google(302 到后端 start)+ /callback(收 code/state→调后端 exchange→写 httpOnly cookie→302 /en/studies;失败→302 /en/login?error=oauth);LoginForm 读 ?error=oauth 显示提示;GoogleButton 点击记 rememberLoginMethod;.env.example 补 OAuth 配置块 + redirect URI 说明 + NEXT_PUBLIC_OAUTH_GOOGLE。新增 tests/integration/test_oauth_google.py(9 用例:start 参数/state 签验/新用户独立 org/两用户 org 各异/老用户复用/同邮箱绑定/SSO 拒密码登录/未配置 503)。验收:ruff 改动文件全绿、后端 179 tests passed(含 9 oauth)、前端 typecheck+build 全绿;真机全链路——点「Continue with Google」经 BFF→后端→真实落 accounts.google.com 且 client_id/redirect_uri/scope/state 参数全对(真实 Google 登录那步交用户在自己浏览器完成)、失败降级 ?error=oauth 显示提示(en「Google sign-in didn't complete」/zh「Google 登录未完成，请重试」)、后端 exchange 拒无效 state 400、BFF callback 无 code/假 code 均 302 回 login?error(不 5xx)、Last used pill 生效、console 0 error、DB 0 脏数据。用户需在 Google Cloud Console 配 redirect URI http://localhost:3300/api/auth/oauth/google/callback。commit ced4b9b

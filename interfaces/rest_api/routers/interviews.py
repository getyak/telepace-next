...
from interfaces.rest_api.deps import get_harness, get_state
...
@router.post("/join")
async def join(body: JoinBody, state=Depends(get_state)) -> dict:
    interview_id = uuid4()
    respondent_id = uuid4()
...
@router.post("/reply")
async def reply(body: ReplyBody, harness: Harness = Depends(get_harness)) -> dict:
    cmd = ReplyInInterview(
        actor=f"interview:{body.interview_id}",
        campaign_id=body.campaign_id,
